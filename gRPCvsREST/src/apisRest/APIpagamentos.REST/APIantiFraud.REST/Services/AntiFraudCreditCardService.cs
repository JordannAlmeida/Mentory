...
namespace APIantiFraud.REST.Services
{
    public class AntiFraudCreditCardService : IAntiFraudCreditCardService
    {
        ...
        public async Task<AntiFraudCreditCardResponse> ValidateAntiFraud(CreditCard credit)
        {
            await Task.Delay(500);
            return GetAntiFraudAvaliation(credit);
        }

        private static AntiFraudCreditCardResponse GetAntiFraudAvaliation(CreditCard credit)
        {
            AntiFraudCreditCardResponse antiFraudCreditCardResponse =
                new()
                {
